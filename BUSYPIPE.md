...
| `bp.idleTimeoutMs` | `120000` | 底层连接无任何 BusyPipe 帧时的空闲超时。 |
| `bp.minJitterBytes` | `8` | MIXED payload 内真实数据偏移的最小抖动距离。 |
| `bp.warmupMs` | `3000` | 握手完成后仅发送随机 `PAD` 的窗口时长，单位 ms，`0` 表示禁用。 |
| `bp.readBufferBytes` | `262144` | 单连接接收侧已解码但未被上层消费的字节上限。超过后 `readLoop` 阻塞，TCP 接收窗口收回，避免内存膨胀。该参数仅本地生效，不参与 HELLO 协商。 |

`bp.idleTimeoutMs` 在协商时取双方较小值。默认已为 `120000`（120s）。若一端仍使用旧默认 15s，协商后两端都会被拖回 15s，大下载时容易误触发 `busypipe: idle timeout`——请确保 **两端** 同步升级或显式配置。
...
这会在监听 socket 上启用 Go 的 `net.ListenConfig.SetMultipathTCP(true)`。

客户端 dialer 同样支持 `mptcp: true`（`bp` 和 `bptls` 均可）：
...
```bash
./gost-bp -L "http://127.0.0.1:18080" \
  -F "relay+bptls://server.example.com:33317?secure=true&serverName=server.example.com&mptcp=true&bp.idleTimeoutMs=120000"
```

客户端启用后会调用 Go 的 `net.Dialer.SetMultipathTCP(true)`，由内核负责协商；服务端不支持或路径不可用时会自动回退到普通 TCP。

注意：客户端 `mptcp=true` 当前会**绕过 chain 的 NetDialer**，因此 `interface` / `netns` / `mark` 等扩展能力不会生效；需要这些能力时请保持 `mptcp=false`。
